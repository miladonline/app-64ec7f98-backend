        ...
        contents=[
            types.TextResourceContents(
                ...
                _meta={
                    ...
                    "openai/widgetDescription": "Simple hello world interactive widget"
                }
            )
        ]
    )
...
async def _list_tools() -> List[types.Tool]:
    ...
    return [
        types.Tool(
            name="say_hello",
            title="Say Hello",
            description="Display a friendly hello world greeting with an optional custom name",
            inputSchema={
                "type": "object",
                "properties": {
                    "name": {
...
async def _call_tool_request(
    ...
    if tool_name == "say_hello":
        ...
        greeting_data = {
            ...
            "timestamp": "now",
            "greeting_styles": [
                {"language": "English", "text": f"Hello, {name}!"},
                {"language": "Spanish", "text": f"¡Hola, {name}!"},
                {"language": "French", "text": f"Bonjour, {name}!"},
                {"language": "German", "text": f"Hallo, {name}!"},
                {"language": "Japanese", "text": f"こんにちは, {name}!"},
            ]
        }
        ...
        return types.CallToolResult(
            content=[
                types.TextContent(
                    type="text",
                    text=f"Generated a friendly greeting for {name}. Explore different language greetings in the interactive widget below.",
                )
            ],
            structuredContent={
                "message": greeting_data["message"],
                "name": name,
                "languages_count": len(greeting_data["greeting_styles"]),
            },
            _meta={
                "fullData": greeting_data,
            },
        )
